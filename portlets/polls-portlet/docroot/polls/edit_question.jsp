		...
		<aui:field-wrapper label="choices">
			...
			for (int i = 1; i <= choicesCount; i++) {
				...
				if (deleteChoice && (i >= choiceName)) {
					paramName = PollsWebKeys.CHOICE_DESCRIPTION_PREFIX + ((char)(96 + i + 1));
				}
				else {
					paramName = PollsWebKeys.CHOICE_DESCRIPTION_PREFIX + c;
				}
			...
			%>
				...
				<div class="choice <%= (i == choicesCount) ? "last-choice" : StringPool.BLANK %>">
					...
					<aui:input name="<%= PollsWebKeys.CHOICE_NAME_PREFIX + c %>" type="hidden" value="<%= c %>" />
...
<aui:script>
	function <portlet:namespace />addPollChoice() {
		<liferay-portlet:renderURL allowEmptyParam="<%= true %>" var="addPollChoiceURL">
			...
			<liferay-portlet:param name="<%= PollsWebKeys.CHOICE_DESCRIPTION_PREFIX + (char)(96 + choicesCount + 1) %>" value="" />
