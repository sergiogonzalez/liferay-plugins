		...
		<td valign="top" width="90%">
			...
			<div class="kb-feedback-actions">
				<c:if test="<%= prevStatus != KBCommentConstants.STATUS_NONE %>">
					...
					<aui:button href="<%= prevStatusURL %>" id="prevStatus" value="<%= KnowledgeBaseUtil.getStatusLabel(prevStatus) %>" />
				</c:if>
				...
				<c:if test="<%= nextStatus != KBCommentConstants.STATUS_NONE %>">
					...
					<aui:button href="<%= nextStatusURL %>" id="nextStatus" value="<%= KnowledgeBaseUtil.getStatusLabel(nextStatus) %>" />
				</c:if>
